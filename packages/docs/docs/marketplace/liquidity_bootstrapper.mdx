...
Strata allows you to bootstrap liquidity selling tokens. This style of sale is done by starting with a high price. 
Then, the price of the token increases for every purchase, and decreases over time.

This idea is borrowed from Balancer LBPs on Etherium, but differs in that there is no initial liquidity needed.
...
Strata LBCs require minimal configuration. All you need to specify is
  * **mint** - What mint would you like to use to collect funds?
