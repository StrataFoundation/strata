...
At the heart of Strata Social Tokens are [Bonding Curves](/docs/concepts/bonding_curves). Bonding Curves allow free and instant trading of a token without the need for an exchange or liquidity provider. A Bonding Curve ties one token to the value of another token.
