...
![Open Collective](/img/OpenSplash.png)
...
The OPEN collective takes no fees. This means that, so long as nobody else has sold, you can sell your tokens for the same amount you put into the curve. 

This also makes the open collective a safe choice for a creator that wants a social token, as there is no middle-man taking a cut.
...
That might mean building a website, setting up a storefront, or just riding on the social network affects of a strong collective.
...
The Open Collective limits royalties on sell of a token to a cumulative 40%. This keeps the authority on a token from completely devaluing the token for all holders. While OPEN tokens do still carry risk, the risk is in token holders selling and not necessarily changes from the authority.
...
While the community that forms around the Open Collective will hopefully guide its values, and drive it forward because of incentive alignment; there is no core team.
...
While not currently supported, the goal is to allow free movement of ones token from one collective to another. This ability may hurt or help the open collective.

## OPEN Fair Launch

The OPEN token will follow a bonding curve with the formula 
