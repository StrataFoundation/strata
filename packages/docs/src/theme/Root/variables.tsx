...
export const VariablesProvider: React.FC = ({ children }) => {
  ...
  const [dependencies, setDependencies] = useState<Record<string, { deps: string[], exec: CodeExec }>>({});
  ...
  const execWithDeps = useMemo(() => async (name: string, vars: any = variables): Promise<any> => {
  ...
  return <VariablesContext.Provider
