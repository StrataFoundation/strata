...
export const MarketplaceItem = ({ tokenBondingKey, name, description, image }: { tokenBondingKey: PublicKey | undefined, name: string, description: string, image: string }) => {
  const { info: tokenBonding, loading: bondingLoading } = useTokenBonding(tokenBondingKey);
  ...
  const { metadata: baseMetadata, loading: metadataLoading } = useTokenMetadata(tokenBonding?.baseMint);
  const { pricing, loading: pricingLoading } = useBondingPricing(tokenBondingKey);
  ...
  const isAdmin = publicKey && (tokenBonding?.reserveAuthority as PublicKey | undefined)?.equals(publicKey);
  ...
  return (
    <VStack p={4} align="start">
      {isAdmin && tokenBondingKey && (
        <MarketplaceItemAdmin tokenBondingKey={tokenBondingKey} />
      )}
      ...
      <Button
        ...
        onClick={() =>
          qtyNumber &&
          qtyNumber > 0 &&
          execute(tokenBondingSdk!, tokenBondingKey!, qtyNumber)
        }
        w="full"
        size="lg"
        colorScheme="blue"
