...
export class SplBondingPresale {
  ...
  async initializePresaleInstructions({
    ...
    presaleBaseStorage,
  }: IInitializePresaleArgs): Promise<
    BigInstructionResult<{ presale: PublicKey }>
  > {
    const instructions1 = [];
    const signers1 = [];
    const instructions2 = [];
    ...
    const tokenBondingAcct =
      await this.tokenBondingProgram.account.tokenBondingV0.fetch(tokenBonding);
    ...
    const presaleEndDate = new Date(
      tokenBondingAcct.goLiveUnixTime.toNumber() * 1000
    );
    ...
    const [baseStorageAuthority, baseStorageAuthorityBumpSeed] =
      await PublicKey.findProgramAddress(
    ...
    if (
      !(tokenBondingAcct.authority! as PublicKey).equals(
        tokenBondingAuthority
      ) ||
      !tokenBondingAcct.buyFrozen
    ) {
      const { instructions: updateInstrs, signers: updateSigners } =
        await this.tokenBondingProgram.updateTokenBondingInstructions({
          tokenBonding,
    ...
    if (!presaleTokenBonding) {
      ...
      const {
        instructions: presaleInstrs,
        signers: presaleSigners,
        output: {
          tokenBonding: out,
          targetMint,
          buyBaseRoyalties,
      ...
      } = await this.tokenBondingProgram.createTokenBondingInstructions({
        payer,
        authority: tokenBondingAuthority,
        curve: presaleCurve!,
      ...
      const preTokenBondingAcct =
        await this.tokenBondingProgram.account.tokenBondingV0.fetch(
          presaleTokenBonding
        );
    ...
    if (!postTokenBonding) {
      const {
        instructions: postInstrs,
        signers: postSigners,
        output: { tokenBonding: out },
      } = await this.tokenBondingProgram.createTokenBondingInstructions({
        payer,
        authority: tokenBondingAuthority,
        curve: postCurve!,
    ...
    instructions4.push(
      await this.instruction.initializeTokenBondingPresaleV0(
        {
          postLaunchTokenBondingAuthority:
            postLaunchTokenBondingAuthority ||
            (tokenBondingAcct.authority as PublicKey)!,
          bumpSeed,
  ...
  async launchInstructions({
    presale,
    refund = this.wallet.publicKey,
  }: ILaunchArgs): Promise<InstructionResult<null>> {
    const presaleAcct = await this.account.tokenBondingPresaleV0.fetch(presale);
    const tokenBondingAcct =
      await this.tokenBondingProgram.account.tokenBondingV0.fetch(
        presaleAcct.tokenBonding
      );
    const presaleTokenBondingAcct =
      await this.tokenBondingProgram.account.tokenBondingV0.fetch(
        presaleAcct.presaleTokenBonding
      );
    const postTokenBondingAcct =
      await this.tokenBondingProgram.account.tokenBondingV0.fetch(
        presaleAcct.postTokenBonding
      );
    const presaleTargetMint = await getMintInfo(
      this.provider,
      presaleTokenBondingAcct.targetMint
    );
    const baseMint = await getMintInfo(
      this.provider,
      tokenBondingAcct.baseMint
    ...
    const presaleBaseStorage = await getTokenAccount(
      this.provider,
      presaleTokenBondingAcct.baseStorage
    );
    const tokenBondingAuthority = await PublicKey.createProgramAddress(
      [
        Buffer.from("token-bonding-authority", "utf-8"),
        presale.toBuffer(),
        new BN(presaleAcct.tokenBondingAuthorityBumpSeed).toBuffer(),
      ],
      this.programId
    );
    const presaleBaseStorageAuthority = await PublicKey.createProgramAddress(
      [
        ...
        presale.toBuffer(),
        new BN(presaleAcct.baseStorageAuthorityBumpSeed).toBuffer(),
      ],
      this.programId
    );
    const curveAcct = await this.tokenBondingProgram.account.curveV0.fetch(
      tokenBondingAcct.curve as PublicKey
    );
        ...
        await this.instruction.launchV0(
          {
            rootEstimates: curve
              .buyWithBaseRootEstimates(
                amountAsNum(presaleBaseStorage.amount, baseMint),
          ...
          },
          {
            accounts: {
              ...
              presaleTargetMintAuthority:
                presaleTargetMint.mintAuthority! as PublicKey,
  ...
  async launch(args: ILaunchArgs): Promise<void> {
    const { instructions, signers } = await this.launchInstructions(args);
