...
import { useCurve, useTokenBonding } from ".";
...
export function useBondingPricing(tokenBonding: PublicKey | undefined): PricingState {
  ...
  const pricing = useMemo(() => curve && base && target && baseStorage && fromCurve(curve, baseStorage, base, target), [curve, baseStorage, base, target])
...
export function useFiatPrice(token: PublicKey | undefined): number | undefined {
  ...
  useEffect(() => {
    if (token?.toBase58() == SOL_TOKEN.toBase58()) {
      setPrice(solPrice);
    } else { // TODO: This is not in sol, so not correct
