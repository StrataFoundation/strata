...
import { ITokenBonding, TokenBondingV0 } from "@strata-foundation/spl-token-bonding";
...
import { getTwitterRegistry, getTwitterRegistryKey } from "../utils/nameServiceTwitter";
...
export async function getTwitterClaimedTokenRefKey(connection: Connection, handle: string, collective: PublicKey = PublicKey.default, tld: PublicKey = WUMBO_TWITTER_TLD): Promise<PublicKey> {
  const owner = (await getTwitterRegistry(connection, handle, tld)).owner;

  return (await PublicKey.findProgramAddress(
    [Buffer.from("token-ref", "utf-8"), owner.toBuffer(), collective.toBuffer()],
    SplTokenCollective.ID
  ))[0];
}
export async function getTwitterUnclaimedTokenRefKey(handle: string, collective: PublicKey = SplTokenCollective.OPEN_COLLECTIVE_ID, tld: PublicKey = WUMBO_TWITTER_TLD): Promise<PublicKey> {
  const name = await getTwitterRegistryKey(handle, tld);

  return (await PublicKey.findProgramAddress(
    [Buffer.from("token-ref", "utf-8"), name.toBuffer(), collective.toBuffer()],
    SplTokenCollective.ID
  ))[0];
}

export const useUnclaimedTwitterTokenRefKey = (name: string | undefined, collective: PublicKey | undefined = SplTokenCollective.OPEN_COLLECTIVE_ID, tld: PublicKey = WUMBO_TWITTER_TLD): { result: PublicKey | undefined, loading: boolean } => {
  const { connection } = useConnection();
  const { result: key, loading } = useAsync(async (name: string | undefined, collective: PublicKey | undefined) => {
      if (connection && name) {
...
export const useClaimedTwitterTokenRefKey = (name: string | undefined, collective: PublicKey = PublicKey.default, tld: PublicKey = WUMBO_TWITTER_TLD): { result: PublicKey | undefined, loading: boolean } => {
  const { connection } = useConnection();
  const { result: key, loading } = useAsync(async (connection: Connection | undefined, name: string | undefined, collective: PublicKey, tld: PublicKey) => {
      if (connection && name) {
...
export const useClaimedTokenRefKey = (owner: PublicKey | undefined, collective: PublicKey = PublicKey.default): PublicKey | undefined => {
  const { result } = useAsync(async (owner: PublicKey | undefined) => owner && SplTokenCollective.tokenRefKey({ owner, collective }), [owner]);
 ...
 * @returns 
 */
export function useReverseTokenRef(mint: PublicKey | undefined): UseAccountState<ITokenRef> {
  const { result: key } = useAsync(async (mint: PublicKey | undefined) =>
 ...
 * @returns 
 */
export const useTwitterTokenRef = (name: string | undefined, collective: PublicKey | undefined, tld: PublicKey = WUMBO_TWITTER_TLD): UseAccountState<ITokenRef> => {
  const {result: claimedKey, loading: twitterLoading } = useClaimedTwitterTokenRefKey(name, tld);
  const { result: unclaimedKey, loading: claimedLoading } = useUnclaimedTwitterTokenRefKey(name, collective, tld);
