...
export class SplTokenAccountSplit {
  ...
  }: ICreateTokenAccountSplitArgs): Promise<
    InstructionResult<{ tokenAccountSplit: PublicKey; tokenAccount: PublicKey }>
  > {
    ...
    const [tokenAccountAuthority, tokenAccountAuthorityBumpSeed] =
  ...
  async createTokenAccountSplit(
    args: ICreateTokenAccountSplitArgs
  ): Promise<{ tokenAccountSplit: PublicKey; tokenAccount: PublicKey }> {
    const { output, instructions, signers } =
      await this.createTokenAccountSplitInstructions(args);
  ...
  async collectTokensInstructions({
    ...
    const tokenAccountSplitAcct = await this.account.tokenAccountSplitV0.fetch(
      tokenAccountSplit
    );
    const tokenStakingAcct =
      await this.splTokenStakingProgram.account.tokenStakingV0.fetch(
        tokenAccountSplitAcct.tokenStaking
      );
    const tokenAccount = await getTokenAccount(
      this.provider,
      tokenAccountSplitAcct.tokenAccount
    );
  ...
  async collectTokens(args: ICollectTokenArgs): Promise<void> {
    const { instructions, signers } = await this.collectTokensInstructions(
      args
    );
    await this.sendInstructions(instructions, signers);
  }
