...
use std::convert::*;
use crate::{arg::{PiecewiseCurve, PrimitiveCurve}, precise_number::{InnerUint, PreciseNumber, ONE_PREC, ZERO_PREC}};
...
impl Curve for PrimitiveCurve {
  fn expected_target_amount(&self, time_offset: i64, base_amount: &PreciseNumber, target_supply: &PreciseNumber, reserve_change: &PreciseNumber, root_estimates: Option<[u128; 2]>) -> Option<PreciseNumber> {
    let guess1 = PreciseNumber { value: InnerUint::from(root_estimates.unwrap()[0]) * 1_000_000_000_000_u64 };
    let guess2 = PreciseNumber { value: InnerUint::from(root_estimates.unwrap()[1]) * 1_000_000_000_000_u64 };
    ...
    if base_amount.eq(&ZERO_PREC) || target_supply.eq(&ZERO_PREC) {
      match self {
        ...
        &PrimitiveCurve::ExponentialCurveV0 { pow, frac, b, c } => {
          let b_prec = PreciseNumber { value: InnerUint::from(b) * 1_000_000_000_000_u64 };
          let c_prec = PreciseNumber { value: InnerUint::from(c) * 1_000_000_000_000_u64 };
          if b == 0 && c != 0 {
            /*
             * (((1 + k) dR)/c)^(1/(1 + k))
             */
      ...
      match self {
        &PrimitiveCurve::ExponentialCurveV0 { pow, frac, b, c } => {
          ...
          if b == 0 && c != 0 {
            ...
            * dS = S dR / R
            */
            target_supply.checked_mul(&reserve_change)?.checked_div(&base_amount)
          } else {
            None // This math is too hard, have not implemented yet.
          }
        }
