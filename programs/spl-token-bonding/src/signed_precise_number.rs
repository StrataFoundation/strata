use crate::{precise_number::{PreciseNumber, InnerUint, HALF, LN2HI_SCALE, LN2LO, TWO_PREC, two, LN2HI, LN2LO_SCALE, ONE_PREC}, uint::U192};
...
pub const P1: SignedPreciseNumber = SignedPreciseNumber { value: PreciseNumber { value: p1() }, is_negative: false };

#[inline]
pub const fn p2() -> InnerUint {
  U192([2777777777701559_u64, 0_u64, 0_u64])
}
pub const P2: SignedPreciseNumber = SignedPreciseNumber { value: PreciseNumber { value: p2() }, is_negative: true };

#[inline]
pub const fn p3() -> InnerUint {
  U192([66137563214379_u64, 0_u64, 0_u64])
}
pub const P3: SignedPreciseNumber = SignedPreciseNumber { value: PreciseNumber { value: p3() }, is_negative: false };

#[inline]
pub const fn p4() -> InnerUint {
  U192([1653390220546_u64, 0_u64, 0_u64])
}
pub const P4: SignedPreciseNumber = SignedPreciseNumber { value: PreciseNumber { value: p4() }, is_negative: true };

#[inline]
pub const fn p5() -> InnerUint {
  U192([41381367970_u64, 0_u64, 0_u64])
}
pub const P5: SignedPreciseNumber = SignedPreciseNumber { value: PreciseNumber { value: p5() }, is_negative: false };
...
impl SignedPreciseNumber {
  ...
  */
  ...
  /// Guarentees 9 decimals of precision
  pub fn exp(&self) -> Option<PreciseNumber> {
    ...
    if self.value.greater_than(&HALFLN2) {
      ...
      if self.value.greater_than_or_equal(&THREEHALFLN2) {
        k = INVLN2.signed().checked_mul(self).unwrap().checked_add(
          &Self {
            value: HALF,
            is_negative: self.is_negative
          }
        ...
        if k.value.to_imprecise().unwrap() > 29 {
          return None
        }
      } else {
        k = Self {
      ...
      hi = self.checked_sub(
          &k.checked_mul(&LN2HI.signed()).unwrap()
            .checked_div(&LN2HI_SCALE.signed()).unwrap()
        ).unwrap();

      lo = k.checked_mul(&LN2LO.signed()).unwrap().checked_div(&LN2LO_SCALE.signed()).unwrap();
    ...
    let p4p5 = P4.checked_add(
      &xx.checked_mul(&P5).unwrap()
    ).unwrap();
    let p3p4p5 = P3.checked_add(
      &xx.checked_mul(&p4p5).unwrap()
    ).unwrap();
    let p2p3p4p5 = P2.checked_add(
      &xx.checked_mul(&p3p4p5).unwrap()
    ).unwrap();
    let p1p2p3p4p5 = P1.checked_add(
      &xx.checked_mul(&p2p3p4p5).unwrap()
    ).unwrap();
    let c = x.checked_sub(
      &p1p2p3p4p5.checked_mul(&xx).unwrap()
    ).unwrap();

    // y = 1. + (x * c / (2. - c) - lo + hi);
    y = ONE_PREC.signed().checked_add(
      &x.checked_mul(&c).unwrap()
       .checked_div(
         &TWO_PREC.signed().checked_sub(&c).unwrap()
        ).unwrap()
    ...
    if k.value.eq(&PreciseNumber::zero()) {
      ...
      let bits = k.value.to_imprecise().unwrap();
...
mod tests {
  ...
  fn test_exp() {
    ...
    // let three_half = PreciseNumber::new(15).unwrap().checked_div(&PreciseNumber::new(10).unwrap()).unwrap().signed();
    ...
    // let point_one = PreciseNumber::new(1).unwrap().checked_div(&PreciseNumber::new(10).unwrap()).unwrap().signed();
    ...
    // let negative = PreciseNumber::new(55).unwrap().checked_div(&PreciseNumber::new(100).unwrap()).unwrap().signed().negate();
    ...
    let test = PreciseNumber::new(23112496541525847408).unwrap().checked_div(&PreciseNumber::new(1000000000000000000).unwrap()).unwrap().signed();
    test.exp().unwrap().print();
    PreciseNumber::new(10905101470688416).unwrap().checked_div(&PreciseNumber::new(1000000).unwrap()).unwrap().print();
    assert!(test.exp().unwrap().almost_eq(
      &PreciseNumber::new(10905101470688416).unwrap().checked_div(&PreciseNumber::new(10000).unwrap()).unwrap(),
      precision
    ));
  }
}