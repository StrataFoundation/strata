...
describe("spl-token-staking", () => {
  ...
  it("Initializes a stake instance and a stake", async () => {
    const tokenStakingAccount = await tokenStakingProgram.account.tokenStakingV0.fetch(
      tokenStaking
    );
  ...
  describe("staking", () => {
    ...
    beforeEach(async () => {
      ...
      stakingVoucherAccount = await tokenStakingProgram.account.stakingVoucherV0.fetch(
        stakingVoucher
      );
    });
    ...
    it("Does not allow unstaking before lockup", async () => {
      expect(tokenStakingProgram.unstake({ tokenStaking, stakingVoucher })).to.eventually.throw(
        /0x131/
      );
    });
